...
//  initialize with a random bisection
void initRandom(louvainPartition *p, adjlist *g) {
  unsigned long i,j,node;
  unsigned long oldComm,newComm;
  ...
  for (unsigned long i = 0; i < p->size - 1; i++) {
    ...
    for (i = 0; i < (g->n)/2 + (g->n)/10; i++) {
      ...
      // insert node in the 1st community
      newComm = 0;
      insertNode(p, g, node, newComm, p->neighCommWeights[newComm]);
    }
    
    for (i = (g->n)/2 + (g->n)/10; i < g->n; i++) {
      node = randomOrder[i];
      oldComm = p->node2Community[node];
      degreeW = degreeWeighted(g, node);
      ...
      // insert node in the 2nd community
      newComm = 1;//p -> node2Community[randomOrder[(g->n)/2]];
      insertNode(p, g, node, newComm, p->neighCommWeights[newComm]);
    }
...
long double louvainOneLevel(louvainPartition *p, adjlist *g) {
  ...
  do {
    ...
    //   compute the gain for its insertion in all neighboring communities
    //   insert it in the best community with the highest gain
    unsigned long nxt[2]= {0,0};
    /*
    for(int i = 0; i < g-> n; i++)
        printf("%d: %d\n", randomOrder[i], p->node2Community[randomOrder[i]]);
    printf("\n");
    */
    for (i = 0; i < g->n; i++) {
        /*for(int i = 0; i < g-> n; i++)
            printf("%d ", randomOrder[i]);
        printf("\n");*/
      //enforce the balance by moving nodes from
      //different communities at every step
      //nbMoves % 2 is the community where we should pick a vertex
      if(nxt[0] == g->n && nxt[1] == g->n)
          break;
      if(nxt[nbMoves%2] == g->n) //all vertices from this com where tried
      {
          nbMoves++, i--;
          continue;
      }
      
      nxt[nbMoves%2] = (i>nxt[nbMoves%2])?i:nxt[nbMoves%2];
      node = randomOrder[ nxt[nbMoves%2]++ ];
      while (p-> node2Community[node] != nbMoves % 2 && nxt[nbMoves%2] < g-> n)
          node = randomOrder[ nxt[nbMoves%2]++ ];
      
      //swap the random order at pos nxt-1 and i
      if(i != nxt[nbMoves%2] -1)
      {
          randomOrder[nxt[nbMoves%2]-1 ] += randomOrder[i];
          randomOrder[i] = randomOrder[ nxt[nbMoves%2]-1 ] - randomOrder[i];
          randomOrder[ nxt[nbMoves%2]-1] -= randomOrder[i];
      }
      //node = randomOrder[i];
      //printf("node %d, com %d, nbMoves %d, nxt0 %d, nxt1 %d\t i %d\n", node, p->node2Community[node], nbMoves, nxt[0], nxt[1], i);
      oldComm = p->node2Community[node];
      degreeW = degreeWeighted(g, node);
      //printf("%d \n", oldComm);
